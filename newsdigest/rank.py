...
class SentIndex:
    ...
    def __init__(self, conn=None, chat_id=""):
        ...
        self.priors = {}
        # и то же самое одной фразой: чем кандидат дополняет ту новость
        # («найдено 200 из 270»). Заполнено там, где событие ТО ЖЕ, а знание
        # прибавилось; у продолжения сюжета новость своя, дополнять ей нечего
        self.gains = {}
        if conn is None:
            return
    ...
    def follow(self, group, prior, gain="") -> None:
        ...
        `gain` — чем именно она ту дополняет, если событие то же самое. Лежит
        рядом со связью, потому что и находится вместе с ней: одним ответом
        модели, одним запросом.
        """
        if not prior:
            return
        for item in group:
            if item["url_hash"] != prior:      # сама себя новость не продолжает
                ...
                if gain:
                    self.gains[item["url_hash"]] = gain

    def prior_of(self, url_hash) -> str:
        """Что эта новость продолжает, если продолжает."""
        return self.priors.get(url_hash, "")

    def gain_of(self, url_hash) -> str:
        """Чем она дополняет ту, которую продолжает. Пусто — событие другое,
        и объяснять читателю разницу не нужно: он её и так видит."""
        return self.gains.get(url_hash, "")
