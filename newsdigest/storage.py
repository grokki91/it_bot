...
CREATE TABLE IF NOT EXISTS dupes (
    ...
    -- сам ответ модели одним словом: same/less/more/next/other (llm.SAME…).
    -- Два поля выше — выжимка из него, и по ним не отличить повтор от
    -- пересказа, который знает МЕНЬШЕ уже показанного. Срочному эта разница
    -- нужна: `more` без единого нового факта будить человека не должно
    kind    TEXT NOT NULL DEFAULT '',
    -- что именно нового во второй новости, до 8 слов. Пусто у всех, кроме
    -- `more`. Из него срочное и делает пометку «что нового»
...
def add_news_gain(conn) -> None:
    """Чем вторая новость дополняет первую (3.9, «что нового»).

    Раньше вердикт был про событие: одно и то же или разные. Но одно и то же
    крушение приходит из пяти лент за час, и пятый пересказ короче первого —
    формально «другое» («лодка затонула» после «лодка перевернулась»), а
    читателю в нём нет ничего. Теперь модель отвечает, что нового узнает
    читатель, и урезанный пересказ отличается от продолжения.

    Пары, рассуженные до появления колонок, остаются с пустым `kind` — это
    «не знаем», и ведут они себя как раньше: `same` и `follows` при них.
    Спрашивать про них заново незачем: они старше `dup_window_h`.
    """
    if not table_exists(conn, "dupes"):  # новая база: колонки придут из SCHEMA
        return
    ensure_column(conn, "dupes", "kind", "TEXT NOT NULL DEFAULT ''")
    ensure_column(conn, "dupes", "gain", "TEXT NOT NULL DEFAULT ''")
