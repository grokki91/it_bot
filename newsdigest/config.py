...
CFG = {
    ...
    # Модель отвечает не «одно ли событие», а «что нового узнает читатель»:
    # same / less (то же, но короче) / more (то же, но счётчик сдвинулся) /
    # next (другое событие того же сюжета) / other. Same и less снимаются
    # везде, а срочному нужна ещё ступень: «то же событие, но есть новое» без
    # НАЗВАННОГО нового будить человека не должно — так одно крушение и
    # приходило дважды за вечер, вторым разом из других лент и короче.
    "dup_breaking_gain": True,     # [env ND_DUP_BREAKING_GAIN] 0 = как раньше:
                                   # срочному довольно, что событие не то же
    ...
    "listen":           True,      # [env ND_LISTEN] принимать из чата нажатия
                                   # кнопок под выпуском (разделы, «Мои темы»,
                                   # «Поделиться») и заявки новых чатов.
    ...
    "chat_reply":       "schedule",# [env ND_CHAT_REPLY] что отвечать на сообщение
                                   ...
                                   # кнопки под выпуском при этом работают)
    ...
    "web_proxy":        False,      # [env ND_WEB_PROXY] перед страницей стоит
                                    # обратный прокси (nginx с TLS на 443):
                                    # только тогда верим его заголовкам
                                    # X-Forwarded-Proto и X-Forwarded-For.
                                    # Без прокси — 0: иначе любой гость назовёт
                                    # себя чужим адресом и обойдёт счётчик
                                    # неверных паролей. Прокси при этом должен
                                    # ставить оба заголовка сам, а страницу
                                    # держите на 127.0.0.1 (ND_WEB_HOST),
                                    # чтобы мимо прокси к ней не ходили
    ...
    # --- обратная связь ------------------------------------------------------
    # 👍/👎/🔖 ставят на странице, под карточками. Из Telegram эти кнопки
    # убраны: ряд из трёх кнопок на каждую новость — это до тридцати кнопок
    # под выпуском, и чем больше новостей, тем сильнее они резали глаз. Там
...
ENV_MAP = {
    ...
    "ND_DUP_BREAKING_GAIN": ("dup_breaking_gain",
                             lambda v: str(v).lower() in ("1", "true", "yes")),
    ...
    "ND_WEB_PROXY": ("web_proxy", lambda v: str(v).lower() in ("1", "true", "yes")),
}
