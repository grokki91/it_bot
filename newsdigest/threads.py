...
событие читателю дважды не нужно. Но самый частый ответ модели — «разные», и
за ним прячутся два очень разных случая:
...
уже проводит: в `dedup` тем же запросом приходит поле `follows`
