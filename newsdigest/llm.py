...
Шкала работает, только если пользоваться ею целиком. Кандидаты в пачке уже
отобраны машиной, приличны все, и прорывом из них может быть один, а не десять.
Поэтому в КАЖДОЙ пачке:
- балл 9 и выше — не больше чем одному кандидату, и только если это правда
  событие дня, а не хорошая новость;
- балл 8 и выше — не больше чем трети пачки;
- медиана пачки должна попасть в 5-7: обычный день состоит из обычных новостей;
- одинаковых баллов не ставь. Два кандидата кажутся равными — решай, какой
  выше, и разводи их хотя бы на 0.1. В этом и состоит работа: расставить, а не
  оценить.
